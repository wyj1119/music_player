...
__align(32) uint8_t mem2base[MEM2_MAX_SIZE] __attribute__((at(0XC1000000)));					//外部SDRAM内存池,前面2M给LTDC用了(1280*800*2)

//内存管理表
uint32_t mem2mapbase[MEM2_ALLOC_TABLE_SIZE] __attribute__((at(0XC1000000+MEM2_MAX_SIZE)));	//外部SDRAM内存池MAP

//内存管理参数	   
const uint32_t memtblsize[SRAMBANK]={MEM2_ALLOC_TABLE_SIZE};	//内存表大小
const uint32_t memblksize[SRAMBANK]={MEM2_BLOCK_SIZE};					//内存分块大小
const uint32_t memsize[SRAMBANK]={MEM2_MAX_SIZE};								//内存总大小
...
{
	return myrealloc(SRAMEX,data_p,new_size);
}
