...
void memmap_flash(void)
{
	...
	QSPI_Send_CMD(W25X_FastReadData,0,(3<<6)|(3<<4)|(3<<2)|(3<<0),8);	//QPI,快速读数据,地址为ReadAddr,4线传输数据_32位地址_4线传输地址_4线传输指令,8空周期,NumByteToRead个数据
...
//	MX_OCTOSPI1_Init();					//初始化QSPI
	NORFLASH_Qspi_Enable();			//使能QSPI模式
	NORFLASH_TYPE=NORFLASH_ReadID();//读取FLASH ID
	if(NORFLASH_TYPE==W25Q256)
	{
		temp=NORFLASH_ReadSR(3);//读取状态寄存器 3，判断地址模式
		if((temp&0X01)==0) //如果不是 4 字节地址模式,则进入 4 字节地址模式
		{
			NORFLASH_Write_Enable();	//写使能
			QSPI_Send_CMD(W25X_Enable4ByteAddr,0,(0<<6)|(0<<4)|(0<<2)|(3<<0),0);//QPI,使能 4 字节地址指令,地址为 0,无数据_8 位地址_无地址_4 线传输指令,无空周期,0 个字节数据
		}
		NORFLASH_Write_Enable();	//写使能
